...
if(isset($_POST['form1'])) {
	...
	if($valid == 1) {
		...
		if($path == '') {
			$statement = $pdo->prepare("UPDATE sliders SET heading=?, content=?, buttonText=?, buttonUrl=?, position=? WHERE id=?");
    		$statement->execute(array($_POST['heading'],$_POST['content'],$_POST['buttonText'],$_POST['buttonUrl'],$_POST['position'],$_REQUEST['id']));
		} else {
        	...
        	$statement = $pdo->prepare("UPDATE sliders SET photo=?, heading=?, content=?, buttonText=?, buttonUrl=?, position=? WHERE id=?");
    		$statement->execute(array($final_name,$_POST['heading'],$_POST['content'],$_POST['buttonText'],$_POST['buttonUrl'],$_POST['position'],$_REQUEST['id']));
		}	   
				...
				<div class="box box-info">
					<div class="box-body">
						...
						<div class="form-group">
							...
							<div class="col-sm-6">
								<input type="text" autocomplete="off" class="form-control" name="buttonText" value="<?php echo $button_text; ?>">
							</div>
						</div>
						<div class="form-group">
							<label for="" class="col-sm-2 control-label">Button URL </label>
							<div class="col-sm-6">
								<input type="text" autocomplete="off" class="form-control" name="buttonUrl" value="<?php echo $button_url; ?>">
							</div>
						</div>
						<div class="form-group">
